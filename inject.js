...
var injected = injected || (function () {
    ...
    var iframe = document.getElementById("_13_WAR_opencpsportlet_dossier-dynamic-form_iframe_");
    function assignValueToInputs(inputName, newText) {
        if (typeof newText === 'string') {
            //input is text, checkbox, radio
            Array.from(iframe.contentWindow.document.querySelectorAll('input')).forEach(el => {
            ...
            Array.from(iframe.contentWindow.document.querySelectorAll('textarea')).forEach(el => {
                if (el.name == inputName) {
                    el.value = newText;
                }
            });
            Array.from(iframe.contentWindow.document.querySelectorAll('select')).forEach(el => {
                if (el.name == inputName) {
                    var val = newText;
                    var opts = el.options;
